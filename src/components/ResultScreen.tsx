...
interface ResultScreenProps {
  ...
  onRedo: () => void;
  onConsolidated: () => void;
  onSaveExtras: (extra: Partial<AreaExtra>) => void;
}
...
export default function ResultScreen({ areaName, result, extras, sessionId, onHome, onRedo, onConsolidated, onSaveExtras }: ResultScreenProps) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-1">Diagnóstico de maturidade de dados</p>
      
      {extras.context && (
        <div className="bg-muted/50 rounded-lg px-4 py-3 mb-4 border border-border/50">
          <p className="text-xs text-muted-foreground mb-0.5 font-medium">Contexto da área</p>
          <p className="text-sm leading-relaxed">{extras.context}</p>
        </div>
      )}
      ...
      <AIActionPlan
        areaName={areaName}
        sessionId={sessionId}
        savedContent={extras.actionPlanContent || ''}
